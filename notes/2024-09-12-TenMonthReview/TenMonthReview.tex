...
SO is aimed at decision problems where there are a large number of alternative solutions so allocating substantial computational resource for simulation to each solution is not practical. SO methods offer intelligent systematic procedures to find optimal solutions efficiently using a finite computational budget for simulation. The time-varying capacity planning problem in homeless care settings does involve a large solution space since housing and shelter capacity may take a wide range of values year on year. In this PhD research we would like to explore to what extent novel efficient MFSO methods can give meaningful analytical support to decision makers in this setting. With this in mind, we now review current SO methods, in particular integer-order and multi-fidelity methods which have particular relevance to our problem. 

\subsubsection{Overview of SO methods}\label{lit-review-so}

In SO, one is typically interested in solving the following optimisation problem: 
%
\begin{equation}\label{SO_general}
    \min \theta(\boldsymbol{x}), \hspace{0.2cm} \boldsymbol{x} \in C
\end{equation}
%
where $\theta(\boldsymbol{x})$ is the true cost of solution $\boldsymbol{x}$ which can only be estimated with stochastic simulation and $C$ is the set of feasible solutions, or the solution space. SO methods can be categorised into those which deal with discrete-valued solution spaces and those which deal with contiuous-valued solution spaces. Stricly speaking, the decision variables in our problem are discrete (the number of housing units or shelter units) however it can be convenient to approximate these variables as continuous-valued (as we do in Section \ref{do}) so here we discuss both discrete and continuous SO methods.

In discrete problems, with a sufficiently small number of feasible solutions, it may be realistic to simulate every solution at least once. In this case, \textbf{ranking and selection} (R\&S) methods are appropriate. Initially each solution is simulated a relatively small number of times, and then a decision is made iteratively about which solutions to discard and which to keep simulating. By the end of the procedure, one has a statistical guarantee on the probabilty that a solution within an `indifference zone' of the true best has been selected (in a frequentist setting).
...
In \textbf{sample average approximation} the optimisation problem is transformed to a deterministic problem by fixing the random numbers used in the stochastic simulation of each feasible solution. The goal is then to minimise the sample average objective function, whose optimal solution is a good approximation of the true optimal solution to the original problem provided the samples are big enough. Of course, one still has to perform the deterministic optimisation, but this can take advantage of some structural problems of the sample average problem such as convexity. 

Finally, we discuss the use of \textbf{meta models} in solving discrete and continuous problems. Meta-model approaches capitalise on the low computational cost of an alternative model of the system in question. This meta model is used to aid the search for a local or global solution. For continuous problems, local meta models usually involve polynomial regression models. These perform well locally but poorly over a large solution space. The gradients of the polynomial meta model is used in the search. Global meta models such as Gaussian process (GP) models, work well in low-dimensional settings where the cost of simulation is high. In Bayesian optimisation (BO), a GP prior distribution of the simulation output is placed over the entire continuous-valued solution space. At each iteration in BO, a sampling decision is made to choose the solution with the best expected improvement on the current best solution and the posterior distribution is subsequently updated. In Section \ref{int-ordered-so} we discuss a discrete version of the GP approach. In \textbf{multi-fidelity simulation optimisation}, one or more low-fidelity meta models of the system are used to aid in the optimisation process. As we discuss in Section \ref{models}, in queueing systems multi-fidelity models are typically available subject to certain assumptions.

\subsubsection{Integer-ordered SO methods} \label{int-ordered-so}
%
In \textbf{discrete stochastic approximation} (DSA) \citep{lim2012stochastic}, the objective function $f$ over the integer-ordered discrete domain is extended an objective function $\tilde{f}$ over a continuous domain. In one-dimensional settings, this extension is done with a simple piecewise linear interpolation. In multiple dimensions, the continuous domain is partitioned into a set of convex hulls covering all integer points. The extended objective function $\tilde{f}(\boldsymbol{x})$ at point $\boldsymbol{x}$ in the continuous domain is constructed as a linear combination of $f$ at each integer point on the convex hull surrounding $\boldsymbol{x}$. Stochastic approximation, as discussed in Section \ref{lit-review-so}, is performed on the continuous domain to move from point to point along the direction of steepest gradient descent. The gradient at $\boldsymbol{x}$ is estimated by performing multiple simulation replications at each integer point on the convex hull surrounding $\boldsymbol{x}$ and taking differences between the resulting estimates of the objective function. At the end of the algorithm, the best solution is rounded to the nearest integer point. The authors define the property of multi-modularity for the objective function $f$ on the discrete domain. Multi-modularity for functions on a discrete domain is the counterpart to convexity for functions on a continuous domain. They show that their algorithm converges to the optimal solution for a multi-modular $f$ and they describe how many queueing systems exhibit this property, making their algorithm particularly suitable for queueing problems.

Another promising method for solving integer-ordered SO problems using gradient information is the retrospective search with piecewise-linear interpolation and neighborhood enumeration \textbf{(R-SPLINE)} \citep{wang2013integer}. While DSA is a stochastic approximation procedure for discrete problems, R-SPLINE can be considered a sample-average approximation procedure for discrete problems. Each iteration of R-SPLINE solves a sample path problem retrospectively (R) using the SPLINE procedure, with a larger number of samples at each simulated solution than the previous iteration. The procedure is retrospective in the sense that each iteration of SPLINE uses the solution from the previous iteration as a `warm-start'. The SPLINE procedure repeatedly performs a search with piecewise linear interpolation (SPLI) and a neighbourhood enumeration (NE). The SPLI step moves from one integer point to another, but to do that the search is `perturbed' onto a point in the continous domain and a gradient at that point is estimated using the same approach as gradient-estimation in DSA. Once the search has travelled in the continuous domain along the direction of steepest gradient descent, the search returns to the nearest integer point and the process is repeated until one of several stopping criteria is met. The NE step checks for any better solution which is separated from the current best by one unit in any dimension. The resulting best solution is taken to the next iteration of SPLINE as a starting solution. Within each iteration of the SPLINE procedure, any solution which is simulated is done using the same number of replications and common random numbers. From one SPLINE iteration to the next, new random numbers are used and an increasing sample size is used, as the algorithm tends to focus on a smaller number of solutions. R-SPLINE converges asymptotically to the set of local optimal solutions and performs competitively in finite-time for a number of real-world problems. 

In recent years, the use of \textbf{Gaussian Markov random fields} (GMRFs) \citep{l2019gaussian} for integer-ordered SO problems has become popular. GMRF-based methods can exploit spatial structure across a multi-dimensional solution space using a neighbourhood approach. A GMRF is a Gaussian random vector where each vector element models the objective function of a feasible solution. The GMRF is defined on an undirected graph comprised of nodes and edges. Each node corresponds to a feasible solution and the edges connecting nodes define the neighbourhood of each feasible solution and the corresponding Markov property. The neighbourhood of a given node is defined as all the nodes which are connected to that node with an edge. The Markov property holds if one can predict the objective function of any feasible solution only with information about the objective functions of the feasible solutions in its neighbourhood. That is, the random variable modelling the objective function of a feasible solution is conditionally independent of all feasible solutions outside of its neighbourhood, conditional on the feasible solutions in its neighbourhood. 

A GMRF is defined by its mean vector and precision matrix. The precision matrix is the inverse of the covariance matrix. Whereas zeros in a covariance matrix correspond to independence between random variables, zeros in a precision matrix correspond to conditional independence between random variables, given all other random variables (see \cite{rue2005gaussian} for details). Assuming that the neighbourhood of each solution is small compared to the total size of the solution space, the precision matrix will be sparse, simplifying computations within this framework. 

The basic outline of a SO algorithm with GMRFs, as detailed by \citet{l2019gaussian}, involves updating the conditional mean vector and precision matrix of the GMRF as more information is collected about specific solutions via stochastic simulation. Following each update, a decision is made about where to simulate next (from a set of candidate solutions) in a manner which balances the exploitation of promising areas of the solution space, and exploring new areas of the solution space. \citet{l2019gaussian} make this decision using a complete expected improvement (CEI) criterion, which is evaluated for each candidate solution. The CEI criterion is similar to the expected improvement (EI) criterion discussed in Section \ref{lit-review-so} for Bayesian optimisation, but it treats both the objective function at the current best solution and that at the candidate solution as a random variable (instead of just the candidate solution as in EI). The algorithm is stopped when the maximum CEI across all solutions is repeatedly below some threshold value and the solution with the lowest conditional mean is returned as the optimal solution. 

\subsubsection{Multi-fidelity SO methods}

As discussed in Section \ref{lit-review-so}, one approach to SO with meta modelling is to use models of multi fidelity in the search. In queueing systems, an analytical queueing model can act as a low-fidelity meta model, as we discuss further in Section \ref{models}. There are several established approaches for incorporating multi-fidelity models into SO, we now discuss three such approaches.

One approach to incorporating multi-fidelity models into SO is to \textbf{perform a deterministic optimisation (DO) first} using a deterministic low-fidelity model. Informally, this helps develop a better understanding of the dynamics of the system in question and of the nature of good solutions. Formally, an optimial solution from a DO procedure can be used as a initial solution for a SO procedure, giving the latter a `head start'.

This approach is explained by \cite{jian2015introduction} in the context of managing bicycle station capacity in a bicycle sharing system at the start of a morning rush hour. Their objective is to minimise customer disatisfaction due to full/empty stations. They first ignore randomness in arrivals and departures of bikes to and from stations and find simple rules of thumb for deciding on the morning capacity of a station depending on whether the station experiences net in-flow or net out-flow. This gives helpful intuition but has serious flaws from ignoring stochasticity, such as assigning no bikes to net in-flow stations. The authors then incorporate stochasticity by modelling the number of bikes at each station $i$ as an $M/M/1/r_i$ queue and including the expected number of full/empty stations in the objective function. This gives more intuitive solutions in situations where the fluid flow model would have preferred a full/empty station. Finally, they capture more system complexity using a discrete-event simulation model. They attempt to solve the resulting SO problem using a random search approach and find that by starting their search at the optimal solution from the queueing model optimisation, they find good solutions with less simulation effort.

Another multi-fidelity modelling approach to SO is known as \textbf{multi-fidelity optimisation with ordinal transformation and optimal sampling} (MO2TOS) and was first proposed by \cite{xu2016mo2tos} and is appropriate for discrete problems. The idea is that low-fidelity evaluations of the entire multi-dimensional solution space can be used to `cheaply' transform solutions onto a new one-dimensional scale where they are ordered from good to bad. Grouping solutions based on their position in the transformed space results in lower group variances and higher inter-group distances than randomly grouping solutions without transformation. These characteristics enables the following optimal sampling scheme to more efficiently search for an optimal solution. The optimal sampling strategy is inherited from the well-known optimcal computing budget allocation rule (OCBA). Subsequent papers have improved the original MO2TOS method by, for example, applying clustering techniques to group solutions in the transformed space to further reduce group variances and further increase inter-group distances \citep{cao2023cluster}. The downside of MO2TOS-based methods is that all feasible solutions must be evaluated with the low fidelity model which may be problematic if the solution space is very large and/or the low-fidelity evaluation is not very cheap. 
