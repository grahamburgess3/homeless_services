...
\item Final chapter (next moves)
\item abstract?
\item Read the whole thing through (does it flow nicely)
\item Send to soups (with update on the 10 month review plan)
\end{itemize}
...
The remainder of this PhD research will address current challenges at the intersection of integer-ordered and multi-fidelity SO. An integer-ordered SO approach is naturally applicable to the time-varying capacity planning problem in the homeless care setting since our decision variables are the numbers of housing/shelter units to open annually. Multi-fidelity methods are attractive given the availability of several suitable multi-fidelity models. We discuss below some initial ideas for contributing to the literature at the intersection of these active fields of research. 

Gradient-based integer-ordered SO methods such as DSA and R-SPLINE expend substantial simulation effort in estimating gradients on the continuous domain. For a given point on the $d$-dimensional continuous domain, all $(d+1)$ integer points on the surrounding simplex must be simulated multiple times to estimate the gradient. This could be more efficient by using information from low-fidelity models when estimating the gradient. One could, for example, use solely a low-fidelity model for objective function estimation at points on the simplex. Alternatively one could use a meta-model definted as a combination of a low-fidelity model and an error term, using some simulation effort to fit this meta-model.

GMRF-based integer-ordered SO methods are popular in the literature but do not typically use multi-fidelity model information. 

Modelling errors of low fidelity models using GMRF.
