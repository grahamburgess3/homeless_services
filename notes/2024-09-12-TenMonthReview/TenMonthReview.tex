...
\item Add more to GMRF bit (the benefits of GMRF and the derivatives thereof (maybe write out the conditional dist...)
\item Add references for the general SO bit ?
...
\citet{l2019gaussian} introduce the Gaussian Markov improvement algorithm (GMIA). The conditional mean vector and precision matrix of the GMRF are updated as more information is collected via simulation. Following each update, a decision is made about where to simulate next using a complete expected improvement (CEI) criterion. The CEI criterion is similar to the expected improvement (EI) criterion discussed in Section \ref{lit-review-so}, but it treats both the objective function at the current best solution and that at each candidate solution as random variables. 
