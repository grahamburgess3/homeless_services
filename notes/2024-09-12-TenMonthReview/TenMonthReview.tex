...
\subsection{Capacity planning in healthcare and homeless settings}

We first discuss modelling approaches in healthcare and homeless settings. Modelling may be used for several purposes including capacity planning. We then discuss specific approaches for capacity planning in these settings. 
...
Capacity planning in healthcare settings typically aims to set staffing levels to meet some performance goal given a time-varying demand for services. Such a goal may be, for example, the probability of a waiting time exceeding some acceptable threshold (tail probability of delay). For a simple queueing system, such as an $M/M/s$ queue, steady-state performance measures may be derived analytically and used to set capacity. With a more complicated $M_t/M/s_t$ queue, the pointwise stationary approximation and simple stationary approximation were proposed to approximate a sufficient capacity level. The square-root staffing rule approximates the average demand on the system using the utilisation of the equivalent infinite-server queue and shows considerable improvement on meeting the desired performance targets. \cite{izady2012setting} use the square-root staffing rule alongside stochastic simulation to meet requirements on waiting times in Accident and Emergency settings. \cite{konrad2023achieving} use a reinforcement learning approach to set staffing levels which satisfy constraints on the tail probability of delay. 

Several studies have looked at optimisation methods for capacity planning for services to support runaway homeless youths (RHYs), each using a different format of objective function. \cite{kaya2022improving} use integer programming (IP) to optimally match demand for support services from RHYs, with supply. They take a cost minimisation approach, where existing capacity is cheaper than potential new capacity. \cite{maass2020optimizing} use IP to optimise the location of new shelters for RHYs and others involved in human traficking. They maximise benefit minus cost, using several metrics for societal benefit such as criminal justice costs avoided and labour productivity gained. \cite{miller2022optimization} tackle a similar problem to \cite{kaya2022improving} but they incorporate societal benefit and maximise a benfit to cost ratio, using a fractional programming method known as Dinkelbach's algorithm. 
